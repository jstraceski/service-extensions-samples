...
def test_jwt_auth_rs256_success(server: CalloutServerTest) -> None:
  with get_insecure_channel(server) as channel:
    ...
    header_value = HeaderValue(
      key='Authorization', raw_value=bytes(authorization_header_value, 'utf-8')
    )
    header_map.headers.extend([header_value])
    ...
    # Instead of directly comparing the full response, check the presence and
    # basic validation of decoded items
    assert 'header_mutation' in str(value)
    for key, expected_value in decoded_items:
      # Check presence of key
      assert key in str(value)
      # For 'iat' and 'exp', check if it matches the pattern since the value
      # will be different
      if key in ['decoded-iat', 'decoded-exp']:
        ...
        assert re.search(
          pattern, str(value)
        ), f'{key} value {expected_value} not found'
